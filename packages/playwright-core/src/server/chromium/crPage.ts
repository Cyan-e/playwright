...
class FrameSession {
    ...
    async _initialize(hasUIWindow: boolean) {
        ...
        if (!isSettingStorageState) {
            ...
            if (options.navigator)
                promises.push(this._updateNavigatorPlatform());
    ...
    async _updateUserAgent(): Promise<void> {
        ...
        await this._client.send('Emulation.setUserAgentOverride', {
            ...
            // userAgentMetadata: my_calculateUserAgentMetadata(options),
        });
    }

    async _updateNavigatorPlatform(): Promise<void> {
        const options = this._crPage._browserContext._options;
        await this._client.send('Emulation.setNavigatorOverrides', {
            platform: options.navigator?.platform ? options.navigator.platform : '',
        });
    }
...
function my_calculateUserAgentMetadata(options: types.BrowserContextOptions) {
    ...
    if (options.navigator) {
        metadata.platform = options.navigator.platform || "";
    }
...
function calculateUserAgentMetadata(options: types.BrowserContextOptions) {
    ...
    if (options.navigator?.userAgentData?.brands)
        metadata.brands = options.navigator.userAgentData.brands;
    if (options.navigator?.userAgentData?.architecture)
        metadata.architecture = options.navigator.userAgentData.architecture;
    if (options.navigator?.userAgentData?.model)
        metadata.model = options.navigator.userAgentData.model;
    if (options.navigator?.userAgentData?.bitness)
        metadata.bitness = options.navigator.userAgentData.bitness;
    if (options.navigator?.userAgentData?.wow64)
        metadata.wow64 = options.navigator.userAgentData.wow64;
    if (options.navigator?.userAgentData?.platformVersion)
        metadata.platformVersion = options.navigator.userAgentData.platformVersion;
    if (options.navigator?.userAgentData?.platform)
        metadata.platform = options.navigator.userAgentData.platform;
